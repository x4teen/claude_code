"""
Improved MNIST CNN: BatchNorm, deeper convs, augmentation, AdamW, LR scheduler,
best-model saving, reproducibility, and DataLoader num_workers.
Run directly or via compare_mnist_models.py.
"""
import torch
import torch.nn as nn
import torch.optim as optim
from torchvision import datasets, transforms
from torch.utils.data import DataLoader
from tqdm import tqdm
import time

# --- Config ---
BATCH_SIZE = 64
EPOCHS = 12
LR = 1e-3
WEIGHT_DECAY = 1e-4
NUM_WORKERS = 2

# --- Data ---
def get_data_loaders(transform_train, transform_test, batch_size=64, num_workers=0):
    train_data = datasets.MNIST("data", train=True,  download=True, transform=transform_train)
    test_data  = datasets.MNIST("data", train=False, download=True, transform=transform_test)
    train_loader = DataLoader(
        train_data, batch_size=batch_size, shuffle=True,
        num_workers=num_workers, pin_memory=False
    )
    test_loader = DataLoader(
        test_data, batch_size=batch_size,
        num_workers=num_workers, pin_memory=False
    )
    return train_loader, test_loader

# --- Model ---
class CNNImproved(nn.Module):
    def __init__(self):
        super().__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.Conv2d(32, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2),
            nn.Dropout2d(0.25),
            nn.Conv2d(32, 64, kernel_size=3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2),
            nn.Dropout2d(0.25),
        )
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(64 * 7 * 7, 256),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(256, 10),
        )

    def forward(self, x):
        return self.classifier(self.features(x))

def run(device=None, seed=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if seed is not None:
        torch.manual_seed(seed)
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(seed)

    transform_train = transforms.Compose([
        transforms.RandomRotation(10),
        transforms.RandomAffine(0, translate=(0.1, 0.1)),
        transforms.ToTensor(),
        transforms.Normalize((0.1307,), (0.3081,)),
    ])
    transform_test = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.1307,), (0.3081,)),
    ])
    train_loader, test_loader = get_data_loaders(
        transform_train, transform_test, BATCH_SIZE, NUM_WORKERS
    )

    model = CNNImproved().to(device)
    optimizer = optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="max", factor=0.5, patience=2
    )
    criterion = nn.CrossEntropyLoss()

    def train_epoch(epoch):
        model.train()
        running_loss = 0.0
        correct = 0
        for images, labels in tqdm(train_loader, desc=f"Epoch {epoch}/{EPOCHS}", leave=False):
            images, labels = images.to(device), labels.to(device)
            optimizer.zero_grad()
            outputs = model(images)
            loss = criterion(outputs, labels)
            loss.backward()
            optimizer.step()
            running_loss += loss.item() * images.size(0)
            correct += (outputs.argmax(1) == labels).sum().item()
        n = len(train_loader.dataset)
        return running_loss / n, correct / n

    def evaluate():
        model.eval()
        correct = 0
        with torch.no_grad():
            for images, labels in test_loader:
                images, labels = images.to(device), labels.to(device)
                correct += (model(images).argmax(1) == labels).sum().item()
        return correct / len(test_loader.dataset)

    best_acc = 0.0
    t0 = time.perf_counter()
    for epoch in range(1, EPOCHS + 1):
        train_epoch(epoch)
        test_acc = evaluate()
        scheduler.step(test_acc)
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), "mnist_cnn_improved_best.pth")
    train_time = time.perf_counter() - t0

    # Load best for final reported accuracy
    model.load_state_dict(torch.load("mnist_cnn_improved_best.pth", map_location=device))
    final_test_acc = evaluate()

    return {
        "test_acc": final_test_acc,
        "best_acc": best_acc,
        "train_time_sec": train_time,
        "epochs": EPOCHS,
    }


if __name__ == "__main__":
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Training improved model on {DEVICE}\n")
    result = run(device=DEVICE, seed=42)
    print(f"\nTest accuracy (best): {result['test_acc']:.2%}")
    print(f"Train time: {result['train_time_sec']:.1f}s")
    print("Best weights saved to mnist_cnn_improved_best.pth")
